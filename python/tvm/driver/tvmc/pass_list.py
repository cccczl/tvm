...
def parse_pass_list_str(input_string):
    ...
    if missing_list := [
        ...
        for p in pass_list
        if len(p.strip()) > 0
        and tvm.get_global_func(_prefix + p.strip(), True) is None
    ]:
        available_list = [
            n[len(_prefix) :] for n in registry.list_global_func_names() if n.startswith(_prefix)
        ]
        raise argparse.ArgumentTypeError(
            f'Following passes are not registered within tvm: {", ".join(missing_list)}. Available: {", ".join(sorted(available_list))}.'
        )
